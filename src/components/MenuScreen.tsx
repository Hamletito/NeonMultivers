  ...
  return (
    <div className="fixed inset-0 z-20 flex flex-col items-center justify-center gap-2 sm:gap-3 pointer-events-auto px-4 py-2 overflow-hidden">
      ...
      {profile?.created && (
        <div className="absolute top-2 left-2 flex items-center gap-1.5">
          <div
            ...
            style={{ backgroundColor: profile.color + '30', borderColor: profile.color }}
          >
      ...
      <div className="flex flex-col gap-1.5 w-40 sm:w-44">
        <button
          onClick={onPlay}
          className="px-4 py-2 bg-primary/20 border border-primary text-primary font-mono text-sm rounded-md hover:bg-primary/30 active:scale-95 transition-all hover:shadow-[0_0_18px_rgba(0,255,204,0.4)]"
        >
          PLAY
        </button>
        ...
        <button
          ...
          className={`px-4 py-1.5 border font-mono text-[11px] rounded-md transition-all flex items-center justify-center gap-1.5 active:scale-95 ${
            state.chaosUnlocked
              ? 'bg-destructive/20 border-destructive/60 text-destructive hover:bg-destructive/30'
              : 'bg-muted/10 border-muted/30 text-muted-foreground hover:border-yellow-400/40 hover:text-yellow-400'
          }`}
        >
          {state.chaosUnlocked ? '🔥 CHAOS' : (<><Lock size={11} />500 🪙</>)}
        </button>

        <button
          onClick={onShop}
          className="px-4 py-1.5 bg-secondary/20 border border-secondary text-secondary font-mono text-[11px] rounded-md hover:bg-secondary/30 active:scale-95 transition-all"
        >
          SHOP
        </button>
        ...
        <button
          ...
          className="px-4 py-1.5 bg-yellow-400/10 border border-yellow-400/60 text-yellow-400 font-mono text-[11px] rounded-md hover:bg-yellow-400/20 active:scale-95 transition-all"
        >
          🏆 ACHIEVEMENTS
        </button>
      </div>

      <p className="text-muted-foreground/50 text-[9px] font-mono mt-1">Tap to jump • Tap again to slam</p>
    </div>
  );
}
